"""
EV Acacia vs EV nano Acacia, both on AgSiNW (SERS) – raw data, 400-1800 cm-1, slide-ready.
...
spectra in the same folder (EV Acacia: S1, S2; EV nano Acacia: s3, s4, s4a).
Bands are matched between the two samples within ±MATCH_CM.

Usage:  python3 EV_Acacia_vs_Nano_AgSiNW.py <acacia_agsinw_dir> <nano_agsinw_dir> [acacia_name] [nano_name]
        (defaults S2 and s4 - the spectra chosen against CaF2 in EV_Acacia_PPT_Compare.py)
"""
import glob
import os
import sys
...
a_pk, a_thr = visible_peaks(acacia, A_NAME, LO, HI)
n_pk, n_thr = visible_peaks(nano, N_NAME, LO, HI)
...
for lab, pk, f in [(f"EV Acacia on AgSiNW ({A_NAME})", a_pk, acacia[A_NAME]["file"]),
                   (f"EV nano Acacia on AgSiNW ({N_NAME})", n_pk, nano[N_NAME]["file"])]:
...
pd.concat([a_pk.assign(spectrum=f"EV Acacia AgSiNW {A_NAME}"),
           n_pk.assign(spectrum=f"EV nano Acacia AgSiNW {N_NAME}")]).to_csv(f"{OUT}/peaks_with_checks.csv", index=False)
...
fig.savefig(f"{OUT}/EV_Acacia_vs_nano_AgSiNW_400-1800.png", dpi=300)
plt.close(fig)
...
fig.text(0.99, 0.005, "Raw data. Labelled peaks exceed the noise-calibrated threshold.", ha="right", fontsize=9,
         color="0.35")
fig.tight_layout(rect=(0, 0.02, 1, 1))
fig.savefig(f"{OUT}/EV_Acacia_vs_nano_AgSiNW_zoom.png", dpi=300)
plt.close(fig)
