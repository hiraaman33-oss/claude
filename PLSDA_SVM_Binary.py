...
obs_score_pls, perm_scores_pls, p_val_pls = permutation_test_score(
    clf_pipe_pls, X_train_sel, y_train,
    scoring='accuracy', cv=cv5,
    n_permutations=N_PERM,
    random_state=RANDOM_STATE, n_jobs=-1)
print("[PLS-DA] Observed : %.2f%%   p = %.4f  [%s]" % (
    obs_score_pls * 100, p_val_pls,
    'SIGNIFICANT (p<0.05)' if p_val_pls < 0.05 else 'NOT significant'))

# Keep backward-compatible alias
obs_score  = obs_score_pls
perm_scores = perm_scores_pls
p_val       = p_val_pls
...
# =============================================================================
# 17. PLS-DA PERMUTATION TEST FIGURE
# =============================================================================
fig, ax = plt.subplots(figsize=(7.5, 4.5))
ax.hist(perm_scores_pls, bins=35, color='lightsteelblue', edgecolor='white',
        label='Permuted accuracy')
ax.axvline(obs_score_pls, color='crimson', lw=2.5,
           label='Observed: %.1f%%   p=%.4f' % (obs_score_pls*100, p_val_pls))
...
# =============================================================================
# 24. SVM-RBF PERMUTATION TEST FIGURE
# =============================================================================
fig, ax = plt.subplots(figsize=(7.5, 4.5))
ax.hist(perm_scores_svm, bins=35, color='moccasin', edgecolor='white',
        label='Permuted accuracy')
ax.axvline(obs_score_svm, color='darkorange', lw=2.5,
...
ax.legend(fontsize=9); ax.grid(True, alpha=0.25)
fig.tight_layout()
save_fig(fig, '17_SVM_Permutation.png')
...
with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
    ...
    rows = [
        ...
        ('PLS-DA Permutation p-value',          '%.4f' % p_val_pls),
    ...
    pd.DataFrame({
        ...
        'Permuted_Accuracy':    perm_scores_pls,
    }).to_excel(writer, sheet_name='PLSDA_Permutation_Test', index=False)

    # SVM Permutation
    pd.DataFrame({
        'Permutation_Index':    np.arange(1, N_PERM+1),
        'Permuted_Accuracy':    perm_scores_svm,
    }).to_excel(writer, sheet_name='SVM_Permutation_Test', index=False)
...
print()
print('  Permutation test results (%d permutations):' % N_PERM)
print('  PLS-DA  p-value : %.4f  [%s]' % (
    p_val_pls, 'SIGNIFICANT' if p_val_pls < 0.05 else 'NOT significant'))
print('  SVM-RBF p-value : %.4f  [%s]' % (
