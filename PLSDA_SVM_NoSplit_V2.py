...
STRATEGY
  - ALL 80 spectra used for model building and validation
  - VIP selection  : 3-LV PLS-DA on all 80  ->  top-2200 VIP
...
OUTPUTS -> C:/Users/Hira Aman/Desktop/PROF_DOMENICO'S/PLSDA_SVM_NoSplit/
  ...
  05_PLSDA_Accuracy_Matrix_5fold.png   <- raw counts (accuracy view)
  06_PLSDA_Accuracy_Matrix_LOO.png     <- raw counts (accuracy view)
  07_PLSDA_Recall_Matrix_5fold.png     <- row-normalised (recall view)
  08_PLSDA_Recall_Matrix_LOO.png       <- row-normalised (recall view)
  09_PLSDA_VIP_Scores.png
  10_PLSDA_Loadings.png
  11_PLSDA_Permutation.png
  12_SVM_Cluster_5fold.png
  13_SVM_Cluster_LOO.png
  14_SVM_Accuracy_Matrix_5fold.png     <- raw counts (accuracy view)
  15_SVM_Accuracy_Matrix_LOO.png       <- raw counts (accuracy view)
  16_SVM_Recall_Matrix_5fold.png       <- row-normalised (recall view)
  17_SVM_Recall_Matrix_LOO.png         <- row-normalised (recall view)
  18_Accuracy_Comparison.png
  19_Recall_Comparison.png
  20_Mean_Spectra.png
...
SG_WINDOW      = 7
SG_POLY        = 2
VIP_TOP_N      = 2200          # <-- CHANGED from 1800 to 2200
N_COMPONENTS   = 5
N_PERM         = 999
RANDOM_STATE   = 42
...
P_load   = pls_full.x_loadings_    # (2200, 5)
...
print("[PLS-DA] LOO-CV on all 80 spectra  (80 folds) ...")
...
print("[SVM-RBF] LOO-CV on all 80 spectra  (80 folds) ...")
...
Z_all   = pca_viz.fit_transform(X_scaled)   # (80, 2)
...
#
#   plot_accuracy_matrix : raw counts  -> diagonal / total  = ACCURACY
#                          Each cell shows the NUMBER of samples.
#
#   plot_recall_matrix   : row-normalised -> diagonal value = RECALL
#                          Each cell shows  fraction  +  count in brackets.
#                          Formula : CM_norm[i,j] = CM[i,j] / sum(CM[i,:])
#                          Diagonal = TP / (TP + FN) = RECALL per class
# =============================================================================

def plot_accuracy_matrix(cm, labels, title, fname, cmap):
    """
    Standard confusion matrix — raw counts.
    Diagonal shows number correctly classified.
    Overall accuracy = trace(CM) / sum(CM).
    """
...
def plot_recall_matrix(cm, labels, title, fname, cmap):
    """
    Recall (row-normalised) confusion matrix.

    Formula applied to every cell:
        CM_norm[i, j] = CM[i, j] / sum(CM[i, :])

    Diagonal interpretation:
        CM_norm[i, i] = TP_i / (TP_i + FN_i)  =  Recall of class i

    Each cell displays:
        decimal value  (e.g. 0.94)
        raw count in brackets  (e.g. (19))
    """
    n_cls    = len(labels)
    row_sums = cm.sum(axis=1, keepdims=True).astype(float)
    cm_norm  = cm.astype(float) / row_sums          # row-normalise

    fig, ax = plt.subplots(figsize=(7, 6))
    im = ax.imshow(cm_norm, interpolation='nearest', cmap=cmap,
                   vmin=0, vmax=1)
    cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label('Recall (fraction)', fontsize=10)
    ...
    # Annotate every cell with value and raw count
    ...
    # Add per-class recall as annotation on the right
    for i, cls in enumerate(labels):
        recall_pct = cm_norm[i, i] * 100
        ax.annotate('Recall=%.0f%%' % recall_pct,
                    xy=(1.01, (n_cls - 1 - i) / (n_cls - 1)),
...
# =============================================================================
# 16. PLS-DA CONFUSION MATRICES
#     Fig 05 & 06  : Accuracy matrix  (raw counts)
#     Fig 07 & 08  : Recall matrix    (row-normalised)
# =============================================================================

# Fig 05 — PLS-DA Accuracy Matrix 5-fold
plot_accuracy_matrix(
    cm_pls_5f, le.classes_,
    'PLS-DA  Accuracy Matrix  |  5-fold CV  (n=80)\n'
    'Accuracy = %.1f%%   |   Cells show sample counts' % (acc_pls_5f * 100),
    '05_PLSDA_Accuracy_Matrix_5fold.png', 'Blues')

# Fig 06 — PLS-DA Accuracy Matrix LOO
plot_accuracy_matrix(
    cm_pls_loo, le.classes_,
    'PLS-DA  Accuracy Matrix  |  LOO-CV  (n=80)\n'
    'Accuracy = %.1f%%   |   Cells show sample counts' % (acc_pls_loo * 100),
    '06_PLSDA_Accuracy_Matrix_LOO.png', 'Purples')

# Fig 07 — PLS-DA Recall Matrix 5-fold
plot_recall_matrix(
    cm_pls_5f, le.classes_,
    'PLS-DA  Recall Matrix  |  5-fold CV  (n=80)\n'
    'Diagonal = Recall per class   |   Formula: TP / (TP + FN)',
    '07_PLSDA_Recall_Matrix_5fold.png', 'Blues')

# Fig 08 — PLS-DA Recall Matrix LOO
plot_recall_matrix(
    cm_pls_loo, le.classes_,
    'PLS-DA  Recall Matrix  |  LOO-CV  (n=80)\n'
    'Diagonal = Recall per class   |   Formula: TP / (TP + FN)',
    '08_PLSDA_Recall_Matrix_LOO.png', 'Purples')
...
fig.tight_layout()
save_fig(fig, '09_PLSDA_VIP_Scores.png')
...
fig.tight_layout()
save_fig(fig, '10_PLSDA_Loadings.png')
...
ax.legend(fontsize=9); ax.grid(True, alpha=0.25)
fig.tight_layout()
save_fig(fig, '11_PLSDA_Permutation.png')

# =============================================================================
# 20. SVM-RBF CLUSTER PLOTS  (PCA space)
# =============================================================================
...
# Fig 12 — SVM 5-fold cluster
fig, ax = plt.subplots(figsize=(8, 6.5))
scatter_cluster_pca(ax, Z_all, classes, y_svm_5f,
    'SVM-RBF Cluster Plot  |  5-fold CV  |  acc=%.1f%%  |  n=80' % (acc_svm_5f*100))
fig.suptitle(TITLE_SUF, fontsize=10)
fig.tight_layout()
save_fig(fig, '12_SVM_Cluster_5fold.png')

# Fig 13 — SVM LOO cluster
fig, ax = plt.subplots(figsize=(8, 6.5))
scatter_cluster_pca(ax, Z_all, classes, y_svm_loo,
    'SVM-RBF Cluster Plot  |  LOO-CV  |  acc=%.1f%%  |  n=80' % (acc_svm_loo*100))
fig.suptitle(TITLE_SUF, fontsize=10)
fig.tight_layout()
save_fig(fig, '13_SVM_Cluster_LOO.png')

# =============================================================================
# 21. SVM CONFUSION MATRICES
#     Fig 14 & 15  : Accuracy matrix  (raw counts)
#     Fig 16 & 17  : Recall matrix    (row-normalised)
# =============================================================================

# Fig 14 — SVM Accuracy Matrix 5-fold
plot_accuracy_matrix(
    cm_svm_5f, le.classes_,
    'SVM-RBF  Accuracy Matrix  |  5-fold CV  (n=80)\n'
    'Accuracy = %.1f%%   |   Cells show sample counts' % (acc_svm_5f * 100),
    '14_SVM_Accuracy_Matrix_5fold.png', 'Blues')

# Fig 15 — SVM Accuracy Matrix LOO
plot_accuracy_matrix(
    cm_svm_loo, le.classes_,
    'SVM-RBF  Accuracy Matrix  |  LOO-CV  (n=80)\n'
    'Accuracy = %.1f%%   |   Cells show sample counts' % (acc_svm_loo * 100),
    '15_SVM_Accuracy_Matrix_LOO.png', 'Purples')

# Fig 16 — SVM Recall Matrix 5-fold
plot_recall_matrix(
    cm_svm_5f, le.classes_,
    'SVM-RBF  Recall Matrix  |  5-fold CV  (n=80)\n'
    'Diagonal = Recall per class   |   Formula: TP / (TP + FN)',
    '16_SVM_Recall_Matrix_5fold.png', 'Blues')

# Fig 17 — SVM Recall Matrix LOO
plot_recall_matrix(
    cm_svm_loo, le.classes_,
    'SVM-RBF  Recall Matrix  |  LOO-CV  (n=80)\n'
    'Diagonal = Recall per class   |   Formula: TP / (TP + FN)',
    '17_SVM_Recall_Matrix_LOO.png', 'Purples')

# =============================================================================
# 22. ACCURACY COMPARISON BAR CHART
# =============================================================================
...
ax.set_title('Overall Accuracy  |  PLS-DA vs SVM-RBF\n'
             'All 80 spectra  |  ' + TITLE_SUF,
             fontsize=11, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.2, axis='y')
fig.tight_layout()
save_fig(fig, '18_Accuracy_Comparison.png')

# =============================================================================
# 23. PER-CLASS RECALL COMPARISON  (2-panel: 5-fold / LOO)
# =============================================================================
n_per_cls = 20          # 80 samples / 4 classes

fig, axes = plt.subplots(1, 2, figsize=(14, 5.5), sharey=True)
panel_data = [
    ('5-fold CV  (n=80)', cm_pls_5f, cm_svm_5f),
    ('LOO-CV  (n=80)',    cm_pls_loo, cm_svm_loo),
]
x = np.arange(4); w = 0.30
for ax, (ptitle, cm_pls, cm_svm) in zip(axes, panel_data):
    rec_pls = [cm_pls[i, i] / n_per_cls * 100 for i in range(4)]
    rec_svm = [cm_svm[i, i] / n_per_cls * 100 for i in range(4)]
    ...
    ax.axhline(100, color='grey', ls=':', lw=1)
    ax.set_xticks(x)
    ax.set_xticklabels([c.replace('SSY-', '') for c in le.classes_],
...
fig.tight_layout()
save_fig(fig, '19_Recall_Comparison.png')
...
ax.set_title('Mean Raman Spectra +/- 1 SD  (all 80 spectra)  |  %d-%d cm-1' % (
    WAVENUMBER_MIN, WAVENUMBER_MAX), fontsize=11, fontweight='bold')
ax.legend(fontsize=9); ax.grid(True, alpha=0.25)
fig.tight_layout()
save_fig(fig, '20_Mean_Spectra.png')
...
with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
    ...
    for i, cls in enumerate(le.classes_):
        cls_rows.append({
            ...
            'PLSDA_5fold_Total':     n_per_cls,
            'PLSDA_5fold_Recall_%':  '%.1f' % (cm_pls_5f[i, i] / n_per_cls * 100),
            'PLSDA_LOO_Correct':     int(cm_pls_loo[i, i]),
            'PLSDA_LOO_Total':       n_per_cls,
            'PLSDA_LOO_Recall_%':    '%.1f' % (cm_pls_loo[i, i] / n_per_cls * 100),
            'SVM_5fold_Correct':     int(cm_svm_5f[i, i]),
            'SVM_5fold_Total':       n_per_cls,
            'SVM_5fold_Recall_%':    '%.1f' % (cm_svm_5f[i, i] / n_per_cls * 100),
            'SVM_LOO_Correct':       int(cm_svm_loo[i, i]),
            'SVM_LOO_Total':         n_per_cls,
            'SVM_LOO_Recall_%':      '%.1f' % (cm_svm_loo[i, i] / n_per_cls * 100),
        })
    ...
    # --- Accuracy confusion matrices (raw counts) ---
    for sheet, cm in [
        ('CM_Accuracy_PLSDA_5fold',  cm_pls_5f),
        ('CM_Accuracy_PLSDA_LOO',    cm_pls_loo),
        ('CM_Accuracy_SVM_5fold',    cm_svm_5f),
        ('CM_Accuracy_SVM_LOO',      cm_svm_loo),
    ]:
        pd.DataFrame(cm,
                     index=pd.Index(le.classes_, name='True\Predicted'),
                     columns=le.classes_).to_excel(writer, sheet_name=sheet)

    # --- Recall confusion matrices (row-normalised) ---
    for sheet, cm in [
        ('CM_Recall_PLSDA_5fold',  cm_pls_5f),
        ('CM_Recall_PLSDA_LOO',    cm_pls_loo),
        ('CM_Recall_SVM_5fold',    cm_svm_5f),
        ('CM_Recall_SVM_LOO',      cm_svm_loo),
    ]:
...
# =============================================================================
print('\n' + '=' * 65)
print('  FINAL RESULTS SUMMARY  (No train/test split  |  VIP top-%d)' % VIP_TOP_N)
print('=' * 65)
...
print()
print('  Per-class Recall  (PLS-DA)  — formula: TP / (TP + FN):')
print('  %-25s  %12s  %10s' % ('Class', '5-fold CV', 'LOO-CV'))
print('  ' + '-' * 50)
for i, cls in enumerate(le.classes_):
    print('  %-25s  %10.0f%%  %8.0f%%' % (
        cls,
        cm_pls_5f[i, i] / n_per_cls * 100,
        cm_pls_loo[i, i] / n_per_cls * 100))
print()
print('  Per-class Recall  (SVM-RBF)  — formula: TP / (TP + FN):')
print('  %-25s  %12s  %10s' % ('Class', '5-fold CV', 'LOO-CV'))
print('  ' + '-' * 50)
for i, cls in enumerate(le.classes_):
    print('  %-25s  %10.0f%%  %8.0f%%' % (
        ...
        cm_svm_loo[i, i] / n_per_cls * 100))
print()
print('  R2X cumulative (PLS-DA):')
for i, r2 in enumerate(r2x_cum, 1):
    print('    LV%d: %.2f%%' % (i, r2 * 100))
print()
print('  Outputs (20 figures + Excel):')
print('    Figs 05-06 : Accuracy Matrix  (raw counts, accuracy view)')
print('    Figs 07-08 : Recall Matrix    (row-normalised, recall per class)')
print('    Figs 14-15 : Accuracy Matrix  SVM-RBF')
print('    Figs 16-17 : Recall Matrix    SVM-RBF')
print()
